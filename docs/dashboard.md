...
- `Integrations`: Postgres, Vault, OIDC, and session-store integration metadata with endpoint/discovery health probes. File-backed sessions are active for local restarts; Postgres session storage is contract-only until a reviewed backend is implemented. Postgres DSNs must not include passwords; the console rejects password-bearing DSNs and redacts any previously saved value before rendering it. OIDC handoff now issues short-lived state/nonce cookies and the callback validates state before failing closed until signed token validation is implemented.
