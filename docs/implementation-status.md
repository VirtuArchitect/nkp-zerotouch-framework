...
| Test Prism connectivity | Implemented baseline | PowerShell and Bash `validate` check endpoint shape/TCP reachability and write `.zt/preflight` evidence; dashboard health can probe Prism API authentication with runtime credentials |
| Test registry connectivity | Implemented baseline | PowerShell and Bash `validate` check endpoint shape/TCP reachability and write `.zt/preflight` evidence; dashboard health can probe registry `/v2/` authentication and `registry -Apply` performs real push |
...
| OIDC login flow | Partial | readiness route validates discovery metadata, builds an authorization handoff with signed state/nonce cookies, and the callback validates the signed handoff before failing closed; full token exchange, signed JWT validation, nonce validation, and role mapping remain future work |
