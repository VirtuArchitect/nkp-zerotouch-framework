...
def test_postgres_dsn_password_is_not_accepted():
    ...
    assert app.postgres_dsn_contains_password(settings["postgres_dsn"])
    assert "secret" not in app.redact_postgres_dsn(settings["postgres_dsn"])
    status, note = app.postgres_status(settings)
    assert status == "warn"
...
def test_integrations_page_redacts_and_rejects_postgres_dsn_password():
    rbac_path = app.SETTINGS / "rbac.json"
    integrations_path = app.SETTINGS / "integrations.json"
    original_rbac = rbac_path.read_text(encoding="utf-8") if rbac_path.exists() else None
    original_integrations = integrations_path.read_text(encoding="utf-8") if integrations_path.exists() else None
    rbac = app.default_rbac()
    account = {
        "username": "integrations-secret-smoke",
        "displayName": "Integrations Secret Smoke",
        "role": "Admin",
        "status": "active",
    }
    account.update(app.password_record("IntegrationsSecretSmoke-Local-123!"))
    rbac["accounts"] = [account]
    app.write_json(rbac_path, rbac)
    app.save_setting("integrations", {
        **app.default_integrations(),
    ...
    try:
        status, _, _ = request(
            ...
            base_url,
            "/login",
            {"username": "integrations-secret-smoke", "password": "IntegrationsSecretSmoke-Local-123!"},
            allow_error=True,
        )
        assert status == 303
        status, _, body = request(opener, base_url, "/settings/integrations")
        assert status == 200
        assert "postgresql://zt_console:***@db/nkp_zerotouch" in body
        ...
        assert status == 400
        assert "must not include a password" in body
        saved = app.read_json(integrations_path)
        assert saved["postgres_dsn"] == "postgresql://zt_console@db/nkp_zerotouch"
    finally:
        server.shutdown()
        server.server_close()
        if original_rbac is None:
